...
// I2C clock frequency is slower. ACK bit of the slave is ignored. Clock
// stretching by the slave is not allowed.
//
...
// The exact number of clock cycles required for jumps and thus also loops
// cannot be precisely predicted. However, this can be accepted for this type of
// application (synchronous data transmission).
//
// F_CPU >= 24000000                                      // ~500kHz I2C clock
#define I2C_DELAY_H()                                                          \
  __asm__("sjmp .+2");                                                         \
...
// I2C macros
#define I2C_SDA_HIGH()                                                         \
  PIN_high(PIN_SDA)                    // release SDA -> pulled HIGH by resistor
#define I2C_SDA_LOW() PIN_low(PIN_SDA) // SDA LOW     -> pulled LOW  by MCU
#define I2C_SCL_HIGH()                                                         \
  PIN_high(PIN_SCL)                    // release SCL -> pulled HIGH by resistor
...
// I2C transmit one data byte to the slave, ignore ACK bit, no clock stretching
// allowed
...
// I2C receive one data byte from the slave (ack=0 for last byte, ack>0 if more
// bytes to follow)
